...
int main(int argc, char **argv)
{
    ...
    paras.uni_sample_radio = 5.0;     // 均匀采样半径比率：采样半径 = 平均边长 × 此比率
    ...
    paras.use_Dynamic_nu = true;        // 启用动态 nu 参数（Welsch 函数的鲁棒性参数逐步缩小）
    ...
    reg->rigid_init(src_mesh, tar_mesh, paras);  // 初始化：构建KD树、查找初始对应关系
